# MOZ_mozambique -- summary

## Overview

- UN status: **Member State**
- Region: Africa / Eastern Africa
- Languages: Portuguese
- Population: 34090466
- GDP per capita: 627 US$ (2025)
- Currencies: MZN
- ccTLD: `.mz` (manager: Centro de Informatica de Universidade Eduardo Mondlane)
- OECD: non-member
- UN/DESA EGDI: Rank 177 of 193
- Government type: presidential republic
- Capital: Maputo
- Hosts collected: 663

## UN/DESA national portal

- Declared: [http://www.portaldogoverno.gov.mz](http://www.portaldogoverno.gov.mz) (host `portaldogoverno.gov.mz`)
- ✅ Covered by collected domains

## Institutions named by CIA Factbook

- Chief of state: President Daniel Francisco CHAPO (since 15 January 2025)
- Head of government: Prime Minister Maria Benvinda Delfina LEVI (since 17 January 2025)
- Highest courts: Supreme Court (consists of the court president, vice president, and 5 judges); Constitutional Council (consists of 7 judges)

(institution names usable as seeds for further research)

## Confirmed domains

1 confirmed domain(s): 1 `central`. Compiled from [`curated.csv`](curated.csv).

| domain | level | source | name |
|---|---|---|---|
| gov.mz | central | curated |  |

## Proposed domains ranked by score

15 domain(s) proposed for validation. Full list in [`proposed.csv`](proposed.csv).
Top 20 by score (0-10) -- higher = stronger cross-source evidence:

| score | hostname | level | http | sources | label |
|------:|----------|-------|------|---------|-------|
| 8 | `parlamento.mz` | central | 301 | wikidata;wikidata | Assembly of the Republic |
| 6 | `cconstitucional.org.mz` | central | 200 | wikidata | Constitutional Council (Mozambique) |
| 4 | `ahm.uem.mz` |  | 200 | wikidata | Arquivo Histórico de Moçambique |
| 4 | `bancomoc.mz` |  | 200 | wikidata | Bank of Mozambique |
| 4 | `mofaic.gov.ae` | central | IOExcepti... | wikidata | Embassy of the United Arab Emirates, Maputo |
| 4 | `stae.org.mz` | central | ConnectEx... | wikidata | Secreteriado Técnico de Administração Eleitoral |
| 3 | `eeas.europa.eu` |  | 200 | wikidata | Delegation of the European Union to Mozambique |
| 3 | `finlandabroad.fi` | central | 301 | wikidata | Embassy of Finland, Maputo |
| 3 | `malawihcmz.com` | central | ConnectEx... | wikidata | High Commission of Malawi, Maputo |
| 3 | `maputo.diplo.de` | central | 200 | wikidata | Embassy of Germany, Maputo |
| 3 | `mozambik.mid.ru` | central | SSLHandsh... | wikidata | Embassy of Russia, Maputo |
| 3 | `mozambique.nlembassy.org` | central | ConnectEx... | wikidata | Embassy of the Netherlands, Maputo |
| 3 | `mz.ambafrance.org` | central | 301 | wikidata | Embassy of France, Maputo |
| 3 | `mz.chineseembassy.org` | central | SSLHandsh... | wikidata | Embassy of the People's Republic of China, Maputo |
| 3 | `swedenabroad.se` | central | 301 | wikidata | Embassy of Sweden, Maputo |

