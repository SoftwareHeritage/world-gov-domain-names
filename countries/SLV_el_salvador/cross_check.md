# Cross-check: SLV_el_salvador

## Overview

- UN status: **Member State**
- ccTLD: `.sv` (manager: SVNet)
- OECD: non-member
- UN/DESA EGDI: Rank 115 of 193
- Government type: presidential republic
- Capital: San Salvador
- Domains collected (HTTP 200): 63

## UN/DESA national portal

- Declared: [https://www.asamblea.gob.sv](https://www.asamblea.gob.sv) (host `asamblea.gob.sv`)
- ⚠️ Exact hostname not in the 200s, but a `gob.sv` root directory exists (to be probed)

## Institutions named by CIA Factbook

- Chief of state: President Nayib Armando BUKELE Ortez (since 1 June 2019)
- Head of government: President Nayib Armando BUKELE Ortez (since 1 June 2019)
- Highest courts: Supreme Court or Corte Suprema de Justicia (consists of 15 judges, including its president, and 15 substitute judges organized into Constitutional, Civil, Penal, and Administrative Conflict Chambers)

(institution names usable as seeds for further research)

## Candidate domains ranked by score

1 candidate(s). Full list in [`candidates.csv`](candidates.csv).
Top 20 by score (0-10) -- higher = stronger cross-source evidence:

| score | hostname | sources | label |
|------:|----------|---------|-------|
| 3 | `jesuitascam.org` | wikidata | Jesuit Province of Central America |

