# Cross-check: GNB_guinea_bissau

## Overview

- UN status: **Member State**
- ccTLD: `.gw` (manager: Autoridade Reguladora Nacional - Tecnologias de Informação e Comunicação da Guiné-Bissau)
- OECD: non-member
- UN/DESA EGDI: Rank 170 of 193
- Government type: semi-presidential republic
- Capital: Bissau
- Domains collected (HTTP 200): 0

## UN/DESA national portal

- Declared: [http://www.gov.gw](http://www.gov.gw) (host `gov.gw`)
- ⚠️ Exact hostname not in the 200s, but a `gov.gw` root directory exists (to be probed)

## Institutions named by CIA Factbook

- Chief of state: Interim President Gen. Horta Nta Na MAN (since 27 November 2025)
- Head of government: Interim Prime Minister Ilídio Vieira TE (since 28 November 2025)
- Highest courts: Supreme Court or Supremo Tribunal de Justica (consists of 9 judges and organized into Civil, Criminal, and Social and Administrative Disputes Chambers)

(institution names usable as seeds for further research)

## Candidate domains ranked by score

1 candidate(s). Full list in [`candidates.csv`](candidates.csv).
Top 20 by score (0-10) -- higher = stronger cross-source evidence:

| score | hostname | sources | label |
|------:|----------|---------|-------|
| 8 | `parlamento.gw` | wikidata;wikidata | National People's Assembly |

