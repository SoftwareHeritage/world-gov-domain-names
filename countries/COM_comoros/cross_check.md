# Cross-check: COM_comoros

## Overview

- UN status: **Member State**
- ccTLD: `.km` (manager: Comores Telecom)
- OECD: non-member
- UN/DESA EGDI: Rank 180 of 193
- Government type: federal presidential republic
- Capital: Moroni
- Domains collected (HTTP 200): 0

## UN/DESA national portal

- Declared: [https://gouvernement.km](https://gouvernement.km) (host `gouvernement.km`)
- ⚠️ Exact hostname not in the 200s, but a `gouvernement.km` root directory exists (to be probed)

## Institutions named by CIA Factbook

- Chief of state: President AZALI Assoumani (since 26 May 2016)
- Head of government: President AZALI Assoumani (since 26 May 2016)
- Highest courts: Supreme Court or Cour Supreme (consists of 7 judges)

(institution names usable as seeds for further research)

## Candidate domains ranked by score

No remaining candidates (every flagged institution is covered).

