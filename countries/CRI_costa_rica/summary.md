# CRI_costa_rica -- summary

## Overview

- UN status: **Member State**
- Region: Americas / Central America
- Languages: Spanish
- Population: 5309625
- GDP per capita: 19970 US$ (2025)
- Currencies: CRC
- ccTLD: `.cr` (manager: National Academy of Sciences (Academia Nacional de Ciencias))
- OECD: member since 2021
- UN/DESA EGDI: Rank 61 of 193
- Government type: presidential republic
- Capital: San José
- Domains collected (HTTP 200): 840

## UN/DESA national portal

- Declared: [https://presidencia.go.cr](https://presidencia.go.cr) (host `presidencia.go.cr`)
- ✅ Covered by collected domains

## Institutions named by CIA Factbook

- Chief of state: President Rodrigo CHAVES Robles (since 8 May 2022)
- Head of government: President Rodrigo CHAVES Robles (since 8 May 2022)
- Highest courts: Supreme Court of Justice (consists of 22 judges organized into 3 cassation chambers each with 5 judges and the Constitutional Chamber with 7 judges)

(institution names usable as seeds for further research)

## Proposed domains ranked by score

16 domain(s) proposed for validation. Full list in [`proposed.csv`](proposed.csv).
Top 20 by score (0-10) -- higher = stronger cross-source evidence:

| score | hostname | level | sources | label |
|------:|----------|-------|---------|-------|
| 6 | `centroamericaribeschp.com` |  | wikidata;wikidata | Piarist province Central America- Caribbean |
| 6 | `jesuitascam.org` |  | wikidata;wikidata | Jesuit Province of Central America |
| 6 | `procomer.com` |  | wikidata;wikidata | Costa Rican Foreign Trade Promoter |
| 4 | `ane.cr` |  | wikidata | Agencia Nacional de Empleo |
| 4 | `bccr.fi.cr` |  | wikidata | Central Bank of Costa Rica |
| 4 | `comisionunesco.cr` |  | wikidata | Costa Rica National Commission for Cooperation with UNESCO |
| 4 | `mofaic.gov.ae` | central | wikidata | Embassy of the United Arab Emirates, San Jose |
| 4 | `sen.inec.cr` |  | wikidata | Sistema Estadístico Nacional |
| 3 | `ccecr.aecid.es` |  | wikidata | Q141100276 |
| 3 | `costarica.mid.ru` | central | wikidata | embassy of Russia in Costa Rica |
| 3 | `cr.ambafrance.org` | central | wikidata | Embassy of France, San José |
| 3 | `cr.chineseembassy.org` | central | wikidata | Embassy of the People's Republic of China, San José |
| 3 | `eeas.europa.eu` |  | wikidata | Delegation of the European Union to Costa Rica |
| 3 | `embocr.com` | central | wikidata | embassy of Bolivia in Costa Rica |
| 3 | `san-jose.diplo.de` | central | wikidata | Embassy of Germany, San José |
| 3 | `san-jose.embassy.qa` | central | wikidata | embassy of Qatar in Costa Rica |

