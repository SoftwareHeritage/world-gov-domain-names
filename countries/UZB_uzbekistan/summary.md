# UZB_uzbekistan -- summary

## Overview

- UN status: **Member State**
- Region: Asia / Central Asia
- Languages: Russian; Uzbek
- Population: 37859698
- GDP per capita: 3968 US$ (2025)
- Currencies: UZS
- ccTLD: `.uz` (manager: Single Integrator for Creation and Support of State Information Systems UZINFOCOM)
- OECD: non-member
- UN/DESA EGDI: Rank 63 of 193
- Government type: presidential republic; highly authoritarian
- Capital: Tashkent (Toshkent)
- Domains collected (HTTP 200): 277

## UN/DESA national portal

- Declared: [https://my.gov.uz](https://my.gov.uz) (host `my.gov.uz`)
- ✅ Covered by collected domains

## Institutions named by CIA Factbook

- Chief of state: President Shavkat MIRZIYOYEV (since 14 December 2016)
- Head of government: Prime Minister Abdulla ARIPOV (since 14 December 2016)
- Highest courts: Supreme Court (consists of 67 judges organized into administrative, civil, criminal, and economic sections); Constitutional Court (consists of 7 judges)

(institution names usable as seeds for further research)

## Proposed domains ranked by score

38 domain(s) proposed for validation. Full list in [`proposed.csv`](proposed.csv).
Top 20 by score (0-10) -- higher = stronger cross-source evidence:

| score | hostname | level | http | sources | label |
|------:|----------|-------|------|---------|-------|
| 7 | `iiv.uz` | central |  | wikidata;wikidata | Ministry of Internal Affairs |
| 7 | `joqargikenes.uz` |  |  | wikidata;wikidata | Supreme Council of Karakalpakstan |
| 7 | `mc.uz` | central |  | wikidata;wikidata | Ministry of Construction of the Republic of Uzbekistan |
| 7 | `mdo.uz` | central |  | wikidata;wikidata | Ministry of Preschool Education of the Republic of Uzbekistan |
| 7 | `mehnat.uz` | central |  | wikidata;wikidata | Ministry of Labor and Social Protection of Population |
| 7 | `mfa.uz` | central |  | wikidata;wikidata | Ministry of Foreign Affairs of Uzbekistan |
| 7 | `minjust.uz` | central |  | wikidata;wikidata | Ministry of Justice |
| 7 | `minzdrav.uz` | central |  | wikidata;wikidata | Ministry of Public Health of Uzbekistan |
| 7 | `motas.uz` | central |  | wikidata;wikidata | Ministry of Tourism and Cultural Heritage |
| 7 | `mudofaa.uz` | central |  | wikidata;wikidata | Ministry of Defense |
| 7 | `uzedu.uz` | central |  | wikidata;wikidata | Ministry of Preschool an School Education of the Republic of Uzbekistan |
| 6 | `sud.uz` | central |  | wikidata | Supreme Court of Uzbekistan |
| 4 | `cbu.uz` |  |  | wikidata | Central Bank of Uzbekistan |
| 4 | `davaktiv.uz` |  |  | wikidata | State Assets Management Agency |
| 4 | `hydroengeo.uz` |  |  | wikidata | Institute of Hydrogeology and Engineering Geology |
| 4 | `ich.uz` |  |  | wikidata | National Commission of the Republic of Uzbekistan for UNESCO |
| 4 | `indembassy.uz` | central |  | wikidata | Embassy of India, Tashkent |
| 4 | `ingeo.uz` |  |  | wikidata | Institute of Geology and Geophysics named after Kh.M. Abdullaev |
| 4 | `kti.iiv.uz` |  |  | wikidata | Research Institute of Criminology of the Republic of Uzbekistan |
| 4 | `meteo.uz` |  |  | wikidata | Centre of Hydrometeorological Service |

