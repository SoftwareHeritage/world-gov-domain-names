# KIR_kiribati -- summary

## Overview

- UN status: **Member State**
- Region: Oceania / Micronesia
- Languages: English; Gilbertese
- Population: 120740
- GDP per capita: 2559 US$ (2025)
- Currencies: AUD; KID
- ccTLD: `.ki` (manager: Ministry of Information, Communications and Transport (MICT))
- OECD: non-member
- UN/DESA EGDI: Rank 147 of 193
- Government type: presidential republic
- Capital: Tarawa
- Domains collected (HTTP 200): 417

## UN/DESA national portal

- Declared: [http://www.president.gov.ki](http://www.president.gov.ki) (host `president.gov.ki`)
- ✅ Covered by collected domains

## Institutions named by CIA Factbook

- Chief of state: President Taneti MAAMAU (since 11 March 2016)
- Head of government: President Taneti MAAMAU (since 11 March 2016)
- Highest courts: High Court (consists of a chief justice and other judges as prescribed by the president)

(institution names usable as seeds for further research)

## Candidate domains ranked by score

1 central-administration candidate(s). Full list in [`candidates.csv`](candidates.csv).
Top 20 by score (0-10) -- higher = stronger cross-source evidence:

| score | hostname | sources | label |
|------:|----------|---------|-------|
| 3 | `ki.chineseembassy.org` | wikidata | Embassy of the People's Republic of China, South Tarawa |

