# MNG_mongolia -- summary

## Overview

- UN status: **Member State**
- Region: Asia / Eastern Asia
- Languages: Mongolian
- Population: 3544835
- GDP per capita: 6751 US$ (2024)
- Currencies: MNT
- ccTLD: `.mn` (manager: Datacom Co., Ltd.)
- OECD: non-member
- UN/DESA EGDI: Rank 46 of 193
- Government type: semi-presidential republic
- Capital: Ulaanbaatar
- Domains collected (HTTP 200): 2226

## UN/DESA national portal

- Declared: [http://www.zasag.mn](http://www.zasag.mn) (host `zasag.mn`)
- ⚠️ ABSENT -- neither `zasag.mn` covered nor `countries/MNG_mongolia/sources/crtsh/zasag.mn.csv` present

## Institutions named by CIA Factbook

- Chief of state: President Ukhnaagiin KHURELSUKH (since 25 June 2021)
- Head of government: Prime Minister Gombojavyn ZANDANSHATAR (since 13 June 2025)
- Highest courts: Supreme Court (consists of the Chief Justice and 24 judges organized into civil, criminal, and administrative chambers); Constitutional Court or Tsets (consists of the chairman and 8 members)

(institution names usable as seeds for further research)

## Proposed domains ranked by score

26 domain(s) proposed for validation. Full list in [`proposed.csv`](proposed.csv).
Top 20 by score (0-10) -- higher = stronger cross-source evidence:

| score | hostname | level | http | sources | label |
|------:|----------|-------|------|---------|-------|
| 10 | `zasag.mn` | central |  | un_desa;wikidata;wikidata;wikidata;wikidata | Government of Mongolia | UN/DESA national portal |
| 9 | `supremecourt.mn` | central |  | wikidata;wikidata | Supreme Court of Mongolia |
| 4 | `flu.mn` |  |  | wikidata | National Influenza Center |
| 4 | `frc.mn` |  |  | wikidata | Financial Regulatory Commission of Mongolia |
| 4 | `imsm.mn` |  |  | wikidata | Institute of Medical Sciences, Mongolia |
| 4 | `m-election.mn` |  |  | wikidata | General Election Commission of Mongolia |
| 4 | `mik.mn` |  |  | wikidata | Mongolian Mortgage Corporation |
| 4 | `mofaic.gov.ae` | central |  | wikidata | Embassy of the United Arab Emirates, Ulaanbaatar |
| 4 | `mongolbank.mn` |  |  | wikidata | Bank of Mongolia |
| 4 | `nationallibrary.mn` |  |  | wikidata | National Library of Mongolia |
| 4 | `nso.mn` |  |  | wikidata | Mongolia National Statistical Committee |
| 4 | `riah.mn` |  |  | wikidata | Research Institute of Animal Husbandry |
| 4 | `ulanbator.mfa.gov.hu` | central |  | wikidata | Embassy of Hungary in Ulaanbaatar |
| 4 | `unesco.mn` |  |  | wikidata | Mongolian National Commission for UNESCO |
| 4 | `vnembassy-ulaanbaatar.mofa.gov.vn​` | central |  | wikidata | Embassy of Vietnam, Ulaanbaatar |
| 3 | `cubadiplomatica.cu` | central |  | wikidata | Embassy of Cuba, Ulaanbaatar |
| 3 | `darkhan.mid.ru` |  |  | wikidata | General Consulate of Russia to Darkhan |
| 3 | `eeas.europa.eu` |  |  | wikidata | Delegation of the European Union to Mongolia |
| 3 | `mfa.bg` | central |  | wikidata | embassy of Bulgaria in Mongolia |
| 3 | `mn.ambafrance.org` | central |  | wikidata | Embassy of France, Ulaanbaatar |

