# ETH_ethiopia -- summary

## Overview

- UN status: **Member State**
- Region: Africa / Eastern Africa
- Languages: Amharic
- Population: 111652998
- GDP per capita: 933 US$ (2025)
- Currencies: ETB
- ccTLD: `.et` (manager: Ethio telecom)
- OECD: non-member
- UN/DESA EGDI: Rank 169 of 193
- Government type: federal parliamentary republic
- Capital: Addis Ababa
- Hosts collected: 712

## UN/DESA national portal

- Declared: [http://www.ethiopia.gov.et](http://www.ethiopia.gov.et) (host `ethiopia.gov.et`)
- ⚠️ Exact hostname not collected, but `gov.et` is harvested (to be probed)

## Institutions named by CIA Factbook

- Chief of state: President TAYE Atske Selassie (since 7 October 2024)
- Head of government: Prime Minister ABIY Ahmed Ali (since April 2018)
- Highest courts: Federal Supreme Court (consists of 11 judges)

(institution names usable as seeds for further research)

## Confirmed domains

1 confirmed domain(s): 1 `central`. Compiled from [`curated.csv`](curated.csv).

| domain | level | source | name |
|---|---|---|---|
| gov.et | central | curated |  |

## Proposed domains ranked by score

30 domain(s) proposed for validation. Full list in [`proposed.csv`](proposed.csv).
Top 20 by score (0-10) -- higher = stronger cross-source evidence:

| score | hostname | level | http | sources | label |
|------:|----------|-------|------|---------|-------|
| 6 | `au.int` |  | 301 | wikidata;wikidata | African Union Commission |
| 6 | `tourismethiopia.org` | central | HttpConne... | wikidata;wikidata | Ministry of Culture and Tourism |
| 4 | `emwa.org.et` |  | 200 | wikidata | Ethiopian Midwives Association (EMwA) |
| 4 | `epaethiopia.org.et` |  | 302 | wikidata | Ethiopian Pharmaceutical Association |
| 4 | `ethiondc.org.et` |  | 200 | wikidata | Ethiopian National Dialogue Commission |
| 4 | `mofaic.gov.ae` | central | IOExcepti... | wikidata | Embassy of the United Arab Emirates, Addis Ababa |
| 4 | `nvi.com.et` |  | 301 | wikidata | National Veterinary Institute |
| 4 | `psi.org.et` |  | 200 | wikidata | Ethiopian Policy Studies Institute |
| 4 | `rrs.et` |  | 200 | wikidata | Refugees and Returnees Service |
| 3 | `addis-abeba.diplo.de` | central | 200 | wikidata | Embassy of Germany, Addis Ababa |
| 3 | `addisabeba.mae.ro` | central | 503 | wikidata | Embassy of Romania, Addis Abeba |
| 3 | `cehro.org` |  | 301 | wikidata | Consortium of Ethiopian Human Rights Organizations |
| 3 | `electionethiopia.org` | central | 301 | wikidata | National Election Board of Ethiopia |
| 3 | `esog-eth.org` |  | 200 | wikidata | Ethiopian Society of Obstetricians and Gynecologists |
| 3 | `et.ambafrance.org` | central | 301 | wikidata | Embassy of France, Addis Ababa |
| 3 | `et.undp.org` |  | 301 | wikidata | United Nations Development Programme |
| 3 | `ethiopia.mid.ru` | central | SSLHandsh... | wikidata | embassy of Russia in Ethiopia |
| 3 | `ethiopianreconciliation.org` |  | ConnectEx... | wikidata | Ethiopian Reconciliation Commission |
| 3 | `etiopien.um.dk` | central | 301 | wikidata | embassy of Denmark in Ethiopia |
| 3 | `finlandabroad.fi` | central | 301 | wikidata | Embassy of Finland, Addis Abeba |

