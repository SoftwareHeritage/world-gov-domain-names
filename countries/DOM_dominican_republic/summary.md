# DOM_dominican_republic -- summary

## Overview

- UN status: **Member State**
- Region: Americas / Caribbean
- Languages: Spanish
- Population: 10771504
- GDP per capita: 11059 US$ (2025)
- Currencies: DOP
- ccTLD: `.do` (manager: Pontificia Universidad Catolica Madre y Maestra, Recinto Santo Tomas de Aquino)
- OECD: non-member
- UN/DESA EGDI: Rank 85 of 193
- Government type: presidential republic
- Capital: Santo Domingo
- Hosts collected: 793

## UN/DESA national portal

- Declared: [http://dominicana.gob.do](http://dominicana.gob.do) (host `dominicana.gob.do`)
- ⚠️ Exact hostname not collected, but `gob.do` is harvested (to be probed)

## Institutions named by CIA Factbook

- Chief of state: President Luis Rodolfo ABINADER Corona (since 16 August 2020)
- Head of government: President Luis Rodolfo ABINADER Corona (since 16 August 2020)
- Highest courts: Supreme Court of Justice or Suprema Corte de Justicia (consists of a minimum of 16 magistrates); Constitutional Court or Tribunal Constitucional (consists of 13 judges)

(institution names usable as seeds for further research)

## Confirmed domains

1 confirmed domain(s): 1 `central`. Compiled from [`curated.csv`](curated.csv).

| domain | level | source | name |
|---|---|---|---|
| gob.do | central | curated |  |

## Proposed domains ranked by score

10 domain(s) proposed for validation. Full list in [`proposed.csv`](proposed.csv).
Top 20 by score (0-10) -- higher = stronger cross-source evidence:

| score | hostname | level | http | sources | label |
|------:|----------|-------|------|---------|-------|
| 7 | `fuerzasarmadas.mil.do` |  | ConnectEx... | wikidata;wikidata | Q17347140 |
| 6 | `centroamericaribeschp.com` |  | 400 | wikidata;wikidata | Piarist province Central America- Caribbean |
| 6 | `dominicoshispania.org` |  | 301 | wikidata;wikidata | Province of Hispania of the Order of Preachers |
| 5 | `bancentral.gov.do` |  | 200 | wikidata | Central Bank of the Dominican Republic |
| 4 | `cesfront.mil.do` | central | 200 | wikidata | Cuerpo Especializado en Seguridad Fronteriza Terrestre |
| 3 | `do.ambafrance.org` | central | 301 | wikidata | Embassy of France, Santo Domingo |
| 3 | `do.china-embassy.org` | central | SSLHandsh... | wikidata | Embassy of the People's Republic of China, Santo Domingo |
| 3 | `eeas.europa.eu` |  | 200 | wikidata | Delegation of the European Union to the Dominican Republic |
| 3 | `nederlandwereldwijd.nl` | central | 301 | wikidata | Embassy of the Netherlands, Santo Domingo |
| 3 | `santo-domingo.diplo.de` | central | 200 | wikidata | Embassy of Germany, Santo Domingo |

