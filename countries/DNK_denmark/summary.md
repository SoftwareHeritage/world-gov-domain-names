# DNK_denmark -- summary

## Overview

- UN status: **Member State**
- Region: Europe / Northern Europe
- Languages: Danish
- Population: 6011488
- GDP per capita: 76970 US$ (2025)
- Currencies: DKK
- ccTLD: `.dk` (manager: Dansk Internet Forum)
- OECD: member since 1961
- UN/DESA EGDI: Rank 1 of 193
- Government type: parliamentary constitutional monarchy
- Capital: Copenhagen
- Domains collected (HTTP 200): 46

## UN/DESA national portal

- Declared: [https://www.regeringen.dk](https://www.regeringen.dk) (host `regeringen.dk`)
- ✅ Covered by collected domains

## Institutions named by CIA Factbook

- Chief of state: King FREDERIK X (since 14 January 2024)
- Head of government: Prime Minister Mette FREDERIKSEN (since 27 June 2019)
- Highest courts: Supreme Court (consists of the court president and 18 judges)

(institution names usable as seeds for further research)

## Proposed domains ranked by score

164 domain(s) proposed for validation. Full list in [`proposed.csv`](proposed.csv).
Top 20 by score (0-10) -- higher = stronger cross-source evidence:

| score | hostname | level | http | sources | label |
|------:|----------|-------|------|---------|-------|
| 10 | `datatilsynet.dk` | central | 200 | wikidata;wikidata;linkgraph | Danish Data Protection Agency |
| 10 | `digst.dk` |  | 200 | wikidata;linkgraph | Agency for Digital Government |
| 10 | `politi.dk` |  | HttpConne... | wikidata;wikidata;linkgraph | Police of Denmark | Rigspolitiet |
| 7 | `advokatnævnet.dk` |  |  | wikidata;wikidata | Disciplinary Board |
| 7 | `at.dk` |  | 200 | wikidata;wikidata | Danish Working Environment Authority |
| 7 | `borger.dk` |  | 301 | linkgraph | Linked from 22 public-sector domains |
| 7 | `bygst.dk` |  | 200 | wikidata;wikidata | Danish Building and Property Agency |
| 7 | `cfcs.dk` |  | 301 | wikidata;wikidata | Centre for Cyber Security |
| 7 | `cpr.dk` |  | 301 | wikidata;wikidata | CPR-Kontoret |
| 7 | `dff.dk` |  | 200 | wikidata;wikidata | Independent Research Fund Denmark |
| 7 | `domaeneklager.dk` |  | 301 | wikidata;wikidata | Danish Complaints Board for Domain Names |
| 7 | `ens.dk` |  | 200 | wikidata;wikidata | Danish Energy Agency |
| 7 | `erhvervsfremmebestyrelsen.dk` |  | 200 | wikidata;wikidata | Danish Board of Business Development |
| 7 | `fanke.dk` |  | 200 | wikidata;wikidata | Danish Financial Complaint Board |
| 7 | `fe-ddis.dk` |  | 301 | wikidata;wikidata | Danish Defence Intelligence Service |
| 7 | `fmi.dk` |  | 301 | wikidata;wikidata | Danish Defence Acquisition and Logistics Organization |
| 7 | `forsyningstilsynet.dk` |  | 200 | wikidata;wikidata | Danish Utility Regulator |
| 7 | `fstyr.dk` |  | 301 | wikidata;wikidata | Danish Road Traffic Authority |
| 7 | `handelskammer.dk` |  | 302 | wikidata;wikidata | German-Danish Chamber of Commerce |
| 7 | `kfst.dk` |  | 200 | wikidata;wikidata | Danish Competition Council | Danish Competition and Consumer Authority |

