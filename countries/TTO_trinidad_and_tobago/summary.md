# TTO_trinidad_and_tobago -- summary

## Overview

- UN status: **Member State**
- Region: Americas / Caribbean
- Languages: English
- Population: 1367764
- GDP per capita: 18733 US$ (2024)
- Currencies: TTD
- ccTLD: `.tt` (manager: University of the West Indies, Faculty of Engineering)
- OECD: non-member
- UN/DESA EGDI: Rank 86 of 193
- Government type: parliamentary republic
- Capital: Port of Spain
- Domains collected (HTTP 200): 610

## UN/DESA national portal

- Declared: [http://www.ttconnect.gov.tt](http://www.ttconnect.gov.tt) (host `ttconnect.gov.tt`)
- ✅ Covered by collected domains

## Institutions named by CIA Factbook

- Chief of state: President Christine KANGALOO (since 20 March 2023)
- Head of government: Prime Minister Kamla Susheila PERSAD-BISSESSAR (since 1 May 2025)
- Highest courts: Supreme Court of the Judicature (consists of a chief justice for both the Court of Appeal with 12 judges and the High Court with 24 judges)

(institution names usable as seeds for further research)

## Proposed domains ranked by score

8 domain(s) proposed for validation. Full list in [`proposed.csv`](proposed.csv).
Top 20 by score (0-10) -- higher = stronger cross-source evidence:

| score | hostname | level | http | sources | label |
|------:|----------|-------|------|---------|-------|
| 4 | `central-bank.org.tt` | central | 301 | wikidata | Central Bank of Trinidad and Tobago |
| 4 | `ema.co.tt` |  | 301 | wikidata | Environmental Management Authority |
| 4 | `tatt.org.tt` |  | 307 | wikidata | Telecommunications Authority of Trinidad and Tobago |
| 3 | `eeas.europa.eu` |  | 200 | wikidata | Delegation of the European Union to Trinidad and Tobago |
| 3 | `port-of-spain.diplo.de` | central | 200 | wikidata | Embassy of Germany, Port of Spain |
| 3 | `tntairports.com` |  | 307 | wikidata | Airports Authority of Trinidad and Tobago |
| 3 | `tt.ambafrance.org` | central | 301 | wikidata | Embassy of France, Port of Spain |
| 1 | `ttparliament.org` | central | HttpTimeo... | wikidata;wikidata;wikidata;wikidata | House of Representatives | Senate of Trinidad and Tobago | Parliament of Trin... |

