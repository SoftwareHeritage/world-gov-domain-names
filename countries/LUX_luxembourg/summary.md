# LUX_luxembourg -- summary

## Overview

- UN status: **Member State**
- Region: Europe / Western Europe
- Languages: German; French; Luxembourgish
- Population: 681973
- GDP per capita: 147252 US$ (2025)
- Currencies: EUR
- ccTLD: `.lu` (manager: RESTENA)
- OECD: member since 1961
- UN/DESA EGDI: Rank 45 of 193
- Government type: constitutional monarchy
- Capital: Luxembourg
- Domains collected (HTTP 200): 19

## UN/DESA national portal

- Declared: [https://www.gouvernement.lu](https://www.gouvernement.lu) (host `gouvernement.lu`)
- ✅ Covered by collected domains

## Institutions named by CIA Factbook

- Chief of state: Grand Duke GUILLAUME (since 3 October 2025)
- Head of government: Prime Minister Luc FRIEDEN (since 17 November 2023)
- Highest courts: Supreme Court of Justice includes Court of Appeal and Court of Cassation (consists of 27 judges on 9 benches); Constitutional Court (consists of 9 members)

(institution names usable as seeds for further research)

## Candidate domains ranked by score

20 central-administration candidate(s). Full list in [`candidates.csv`](candidates.csv).
Top 20 by score (0-10) -- higher = stronger cross-source evidence:

| score | hostname | sources | label |
|------:|----------|---------|-------|
| 7 | `cssf.lu` | wikidata;wikidata | Commission de Surveillance du Secteur Financier |
| 4 | `anlux.lu` | wikidata | National Archives of Luxembourg |
| 4 | `bcl.lu` | wikidata | Central Bank of Luxembourg |
| 4 | `cpll.lu` | wikidata | Permanent Council of the Luxembourgish |
| 4 | `etat.lu` | linkgraph | Linked from 4 public-sector domains |
| 4 | `geologie.lu` | wikidata | Q78188560 |
| 4 | `lih.lu` | wikidata | Luxembourg Institute of Health |
| 4 | `lu.usembassy.gov` | wikidata | Embassy of the United States, Luxembourg |
| 4 | `luxinnovation.lu` | wikidata | Luxinnovation |
| 4 | `mnaha.lu` | wikidata | National Museum of Archeology, History and Art |
| 4 | `railinfra.lu` | wikidata | Administration des chemins de fer |
| 3 | `ambruslu.mid.ru` | wikidata | Embassy of Russia in Luxembourg |
| 3 | `lu.china-embassy.org` | wikidata | Embassy of the People's Republic of China, Luxembourg City |
| 3 | `luxembourg.europarl.europa.eu` | wikidata | European Parliament Liaison Office in Luxembourg |
| 3 | `luxembourg.mae.ro` | wikidata | Embassy of Romania, Luxembourg |
| 3 | `luxembourg.representation.ec.europa.eu` | wikidata | European Commission Representation in Luxembourg |
| 3 | `luxemburg.diplo.de` | wikidata | Embassy of Germany, Luxembourg |
| 3 | `luxemburgo.embaixadaportugal.mne.pt` | wikidata | Embassy of Portugal in Luxembourg |
| 3 | `mzv.cz` | wikidata | Embassy of the Czech Republic in Luxembourg |
| 1 | `beneluxparl.eu` | wikidata;wikidata;wikidata;wikidata | Benelux Parliament |

## Local / regional candidates

12 candidate(s) attributed to a non-central administration (out of the registry's central-gov scope). Full list in [`candidates-local.csv`](candidates-local.csv). Top 10:

| score | hostname | sources | label |
|------:|----------|---------|-------|
| 4 | `uni.lu` | wikidata | University of Luxembourg |
| 6 | `curia.europa.eu` | wikidata;wikidata;wikidata | General Court | European Court of Justice | Court of Justice of the European ... |
| 6 | `ec.europa.eu` | wikidata;wikidata;wikidata | Eurostat |
| 4 | `kayl.lu` | wikidata | Kayl communal council |
| 4 | `mondercange.lu` | wikidata | Mondercange communal council |
| 4 | `petange.lu` | wikidata | Pétange communal council |
| 4 | `schifflange.lu` | wikidata | Schifflange communal council |
| 3 | `cdt.europa.eu` | wikidata | Translation Centre for the Bodies of the European Union |
| 3 | `courbeneluxhof.be` | wikidata | Benelux Court of Justice |
| 3 | `eca.europa.eu` | wikidata | European Court of Auditors |

