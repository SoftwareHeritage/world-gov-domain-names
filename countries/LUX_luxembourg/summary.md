# LUX_luxembourg -- summary

## Overview

- UN status: **Member State**
- Region: Europe / Western Europe
- Languages: German; French; Luxembourgish
- Population: 681973
- GDP per capita: 147252 US$ (2025)
- Currencies: EUR
- ccTLD: `.lu` (manager: RESTENA)
- OECD: member since 1961
- UN/DESA EGDI: Rank 45 of 193
- Government type: constitutional monarchy
- Capital: Luxembourg
- Hosts collected: 391

## UN/DESA national portal

- Declared: [https://www.gouvernement.lu](https://www.gouvernement.lu) (host `gouvernement.lu`)
- ✅ Covered by collected domains

## Institutions named by CIA Factbook

- Chief of state: Grand Duke GUILLAUME (since 3 October 2025)
- Head of government: Prime Minister Luc FRIEDEN (since 17 November 2023)
- Highest courts: Supreme Court of Justice includes Court of Appeal and Court of Cassation (consists of 27 judges on 9 benches); Constitutional Court (consists of 9 members)

(institution names usable as seeds for further research)

## Confirmed domains

4 confirmed domain(s): 4 `central`. Compiled from [`curated.csv`](curated.csv).

| domain | level | source | name |
|---|---|---|---|
| chd.lu | central | curated |  |
| gouvernement.lu | central | curated |  |
| men.public.lu | central | curated |  |
| public.lu | central | curated |  |

## Proposed domains ranked by score

19 domain(s) proposed for validation. Full list in [`proposed.csv`](proposed.csv).
Top 20 by score (0-10) -- higher = stronger cross-source evidence:

| score | hostname | level | http | sources | label |
|------:|----------|-------|------|---------|-------|
| 7 | `cssf.lu` |  | 200 | wikidata;wikidata | Commission de Surveillance du Secteur Financier |
| 4 | `anlux.lu` |  | SSLHandsh... | wikidata | National Archives of Luxembourg |
| 4 | `bcl.lu` |  | 301 | wikidata | Central Bank of Luxembourg |
| 4 | `cpll.lu` |  | 500 | wikidata | Permanent Council of the Luxembourgish |
| 4 | `etat.lu` |  | 302 | linkgraph | Linked from 4 public-sector domains |
| 4 | `geologie.lu` |  | 200 | wikidata | Q78188560 |
| 4 | `lih.lu` |  | SSLHandsh... | wikidata | Luxembourg Institute of Health |
| 4 | `luxinnovation.lu` |  | 200 | wikidata | Luxinnovation |
| 4 | `mnaha.lu` |  | 301 | wikidata | National Museum of Archeology, History and Art |
| 4 | `railinfra.lu` |  | 302 | wikidata | Administration des chemins de fer |
| 3 | `ambruslu.mid.ru` | central | SSLHandsh... | wikidata | Embassy of Russia in Luxembourg |
| 3 | `lu.china-embassy.org` | central | SSLHandsh... | wikidata | Embassy of the People's Republic of China, Luxembourg City |
| 3 | `luxembourg.europarl.europa.eu` | central | 302 | wikidata | European Parliament Liaison Office in Luxembourg |
| 3 | `luxembourg.mae.ro` | central | ConnectEx... | wikidata | Embassy of Romania, Luxembourg |
| 3 | `luxembourg.representation.ec.europa.eu` |  | 302 | wikidata | European Commission Representation in Luxembourg |
| 3 | `luxemburg.diplo.de` | central | 200 | wikidata | Embassy of Germany, Luxembourg |
| 3 | `luxemburgo.embaixadaportugal.mne.pt` | central | IOExcepti... | wikidata | Embassy of Portugal in Luxembourg |
| 3 | `mzv.cz` | central | 302 | wikidata | Embassy of the Czech Republic in Luxembourg |
| 1 | `beneluxparl.eu` |  | 302 | wikidata;wikidata;wikidata;wikidata | Benelux Parliament |

